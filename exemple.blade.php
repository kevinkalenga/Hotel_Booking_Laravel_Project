...
document.addEventListener("DOMContentLoaded", function() {
    ...
    paypal.Buttons({
        ...
        // ✅ Étape 1 : Créer la commande côté client
        createOrder: function(data, actions) {
            return actions.order.create({
                purchase_units: [{
                    amount: {
                        value: totalAmount // total corrigé en float
                    }
                }]
            });
        },

        // ✅ Étape 2 : Capturer le paiement avant de rediriger
        onApprove: function(data, actions) {
            return actions.order.capture().then(function(details) {
                console.log('Payment captured:', details); // pour debug

                // Récupère l’ID de la capture
